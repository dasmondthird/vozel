...
func (b *Bot) LoadTariffs() {
	tariffs := []Tariff{
		{Name: "Basic", Price: 100, Duration: 30 * 24 * time.Hour},
		{Name: "Premium", Price: 300, Duration: 90 * 24 * time.Hour},
		{Name: "Ultimate", Price: 1000, Duration: 365 * 24 * time.Hour},
	}
...
func (b *Bot) AddTariffButtons() {
	for name, tariff := range b.Tariffs {
		btn := b.Menus.TariffMenu.Data(name+" - "+strconv.Itoa(tariff.Price)+" руб", "tariff_"+name)
...
func main() {
	...
	scheduler := NewScheduler(botInstance)
	scheduler.Start()
}
